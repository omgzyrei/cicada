...
function Disable-AllSecurity {
    ...
    Write-Host "`n** RESTART REQUIRED FOR CHANGES TO TAKE EFFECT **" -ForegroundColor DarkRed
    $restart = Read-Host "`nWould you like to restart now? (Y/N)"
    if ($restart -eq 'Y' -or $restart -eq 'y') {
        Write-Host "`nRestarting computer in 5 seconds..." -ForegroundColor Yellow
        Write-Host "Press CTRL+C to cancel" -ForegroundColor Yellow
        Start-Sleep -Seconds 5
        Restart-Computer -Force
    } else {
        Write-Host "`nRemember to restart your computer later for changes to take effect." -ForegroundColor Yellow
    }
}
...
} while ($choice -ne '5')Flags"; Value = 0},
...
function Enable-AllSecurity {
    ...
    $allSettings = @(
        ...
        @{Path = "HKLM:\SYSTEM\CurrentControlSet\Control\Lsa"; Name = "LsaCfg
